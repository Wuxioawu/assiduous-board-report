...
Claude Code must NOT run any git commands (add, commit, push, branch, etc.) in this
project. All git operations (staging, committing, pushing, branching, tagging) are
performed manually by the developer. Claude Code may suggest what commands to run
(per CONTRIBUTING.md conventions) but must not execute them.