...
def compute_growth_metrics(
    ...
    results.append(MetricResult("revenue", revenue, None if revenue is not None else "Revenue not extracted"))
    ...
    results.append(
        MetricResult(
            "customer_count", customer_count, None if customer_count is not None else "Customer count not extracted"
        )
    )
